...
(defalias 'frame-get-window
  'frame-first-window
  "Return the main (upper right) window in FRAME")
...
;; ------------------------------------------------------------------------- ;;
;; Creating and Focusing Frames
;; ------------------------------------------------------------------------- ;;
(defalias 'frame-new 'make-frame
  "Creates a new frame")

(defalias 'frame-display 'raise-frame
  "Brings the given frame ARG1 to the front and selects it.")

(defun frame-raise (&optional frame ignore-selected)
  "Brings the given FRAME to the front.

Unlike `frame-display' / `raise-frame', this function will only
change the order of the frames without changing the focus of the
current frame. If IGNORE-SELECTED is non-nil, FRAME will be above
the selected frame possibly obscuring it."
  (frame-restack frame (selected-frame) ignore-selected))
...
;; ------------------------------------------------------------------------- ;;
(defun frame-add-to-display-buffer-alist
    (condition &optional parameters functions)
  "Quick function to add / remove keys from `display-buffer-alist'

Deletes any elements with CONDITION first and if PARAMETERS are
non-nil, adds (CONDITION FUNCTIONS PARAMETERS) to
`display-buffer-alist'. If not given, FUNCTIONS default to
(`display-buffer-reuse-window' `display-buffer-pop-up-frame')."
  (unless functions
    (setq functions
          #'(display-buffer-reuse-window display-buffer-pop-up-frame)))

  (setq display-buffer-alist (assoc-delete-all condition display-buffer-alist))

  (when parameters
    (push (cons condition (append (list functions) parameters))
...
;; Miscellaneous 
;; ------------------------------------------------------------------------- ;;
(defun window-goto-end (window)
  "Moves the point to the end of the buffer displayed in WINDOW"
  (set-window-point
   window
   (1+ (buffer-size (window-buffer window)))))

(defun window-list-with-buffer (&optional buffer)
  "Returns list of all windows with BUFFER (current by default)"
  (get-buffer-window-list buffer nil t))

(defun window-buffer-exclusive (&optional window)
  "Returns t if the buffer WINDOW (current by default) is
displaying is the only window."
  (get-buffer-window-list buffer nil t))




(defun buffer-goto-end (buffer &optional all)
  "Moves the point to the end of the BUFFER"
  (if all
      (-each (get-buffer-window-list buffer nil t)
	    #'window-goto-end)
    (window-goto-end (get-buffer-window buffer t))))

(defun buffer-window-count (&optional buffer)
  "Returns the number of windows with BUFFER (current by default)"
  (length (window-list-with-buffer buffer)))
